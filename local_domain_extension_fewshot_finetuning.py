...
def main(args):
    ...
    if args.k > 0:
        ...
        dictionary = full_corpus.make_label_dictionary("ner")
        tars_tagger.add_and_switch_to_new_task(
            task_name="zeroshot-short", label_dictionary=dictionary, label_type="ner"
        )
        ...
        result = tars_tagger.evaluate(
            data_points=full_corpus.test,
            gold_label_type="ner",
            out_path=f"{base_save_path}/predictions.txt",
        )
        with open(
            f"{base_save_path}/result.txt",
            "w",
        ) as f:
            f.write(result.detailed_results)
